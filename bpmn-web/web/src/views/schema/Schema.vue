        ...
        <template #default="{ node, data }">
          <span class="tree-node-content">
            <span
              ...
              :class="{ 'not-valid': data.notValid }"
              >{{ node.label }}</span
            >
            <span class="tree-node-type" v-if="data.type">{{
...
const validateCurrentNodeValue = () => {
  ...
  currentNodeValue.value &&
    (currentNodeValue.value.notValid = !schemaValidater(
      currentNodeValue.value
    ));
};
...
.not-valid {
  border-left: 8px var(--el-color-danger) solid;
  padding-left: 3px;
}
